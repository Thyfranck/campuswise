class Jobs::ReminderJob < Struct.new(:payment)
  ...
  def send_dashboard_notifications
    ...
    @lender_dashboard_notification =  DashboardNotification.new(
      ...
      :exchange_id => payment.exchange.id,
      :content => "Please inform us when and wheather the book titled \"<a href='/books/#{payment.exchange.book.id}' target='_blank'> #{payment.exchange.book.title.truncate(25)} </a> \" is returned.")
